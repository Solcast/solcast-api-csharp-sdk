...
```csharp
var response = await client.Data.Tmy.Advanced_pv_power.GetAsync(request =>
{
...
| Period | Period2 | No | Length of the averaging period in ISO 8601 duration format. Default is PT60M. |
| GhiWeight | number | No | When creating the TMY, the weighting of GHI to use in the target parameter. Note that ghi_weight + dni_weight must equal 1. |
| DniWeight | number | No | When creating the TMY, the weighting of DNI to use in the target parameter. Note that ghi_weight + dni_weight must equal 1. |
| Probability | Probability | No | The probability percentile for the TMY. |
...
**Returns** `TmyAdvancedPvPowerResponse`

| Field | Description |
| --- | --- |
| estimated_actuals | Typical Meteorological Year Advanced PV power records. |
...
```csharp
var response = await client.Data.Tmy.Radiation_and_weather.GetAsync(request =>
{
...
| DniWeight | number | No | When creating the TMY, the weighting of DNI to use in the target parameter. Note that ghi_weight + dni_weight must equal 1. |
| Probability | Probability | No | The probability percentile for the TMY. |
| Format | Format3 | No | Response format. Default is HTML if not supplied. |
...
**Returns** `TmyRadiationAndWeatherResponse`

| Field | Description |
| --- | --- |
| estimated_actuals | Typical Meteorological Year irradiance and weather records. |
...
```csharp
var response = await client.Data.Tmy.Rooftop_pv_power.GetAsync(request =>
{
    request.QueryParameters.Latitude = latitude;
    request.QueryParameters.Longitude = longitude;
...
**Returns** `TmyRooftopPvPowerResponse`

| Field | Description |
| --- | --- |
| estimated_actuals | Typical Meteorological Year rooftop PV power records. |
